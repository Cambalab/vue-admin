...
describe('Unauthorized User Test', () => {
  ...
  const authResponseWithUserWithoutPermissions = createAuthResponse(
    userWithoutPermissions
  )
  const authResponseWithUserWithPermissions = createAuthResponse(
    userWithPermissions
  )
  ...
  it('A user without admin privileges should be redirected when trying to access an unauthorized view', () => {
    cy.InitAuthenticatedUser({
      authResponse: authResponseWithUserWithoutPermissions,
    })
  ...
  it('A user with admin privileges should not be redirected when trying to access an unauthorized view', () => {
    cy.InitAuthenticatedUser({
      authResponse: authResponseWithUserWithPermissions,
    })
    cy.visit(`/#/${testResourceName}/`)
