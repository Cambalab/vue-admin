...
describe('Auth Test', () => {
  ...
  it('When a user types an unexisting username, the snackbar should be displayed and contain a feedback message', () => {
    findTypeAndAssert({
      element: UI_NAMES.AUTH_USERNAME_INPUT,
      value: 'non_existing@camba.coop',
      condition: 'have.value',
    })
    findTypeAndAssert({
      element: UI_NAMES.AUTH_PASSWORD_INPUT,
    ...
    const snackbar = cy.getElement({
      constant: UI_NAMES.AUTH_SNACKBAR,
      elementType: 'div',
      elementProp: 'name',
    })
    snackbar.should('be.visible')
    const invalidUserPasswordText =
      UI_CONTENT.AUTH_SNACKBAR_INVALID_USER_PASSWORD
    snackbar.should('contain', invalidUserPasswordText)
  })

  it('When a user types an incorrect password, the snackbar should be displayed and contain a feedback message', () => {
    findTypeAndAssert({
      element: UI_NAMES.AUTH_USERNAME_INPUT,
      value: user.username,
      condition: 'have.value',
